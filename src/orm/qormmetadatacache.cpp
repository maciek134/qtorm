        ...
        for (int i = startFrom; i < data.size(); ++i)
        {
            if (data[i].isUpper())
            {
                for (auto it = std::cbegin(std::forward<Iterable>(keywordDescriptors));
                     ...
                     ++it)
                {
                    if (data.mid(i, it->token.size()) == it->token)
                    {
                        return {i, it};
                    }
                }
    ...
    const QOrmUserMetadata& userPropertyMetadata)
{
    ...
    descriptor.dataType = property.type() == QVariant::UserType
                              ...
                              : static_cast<QMetaType::Type>(property.type());
#else
    descriptor.dataType = static_cast<QMetaType::Type>(property.metaType().id());
#endif
    ...
    if (descriptor.dataType >= QMetaType::User)
    {
        ...
        else if (flags.testFlag(QMetaType::IsEnumeration))
        {
            ...
            descriptor.dataType = QMetaType::Int;
        }
    }
